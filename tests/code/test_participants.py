...
def test_addPersonToEvent():
    with mainDB.atomic() as transaction:
        ...
        assert not checkUserVolunteer(
            user, newEvent
        ), "A Volunteer record was added instead"
...
def test_updateEventParticipants():
    ...
    participantData = {
        "inputHours_agliullovak": 100,
        "checkbox_agliullovak": "on",
        "event": 3,
        "username1": "agliullovak",
    }
...
def test_trainedParticipants():
    ...
    with mainDB.atomic() as transaction:
        ...
        assert (
            attendedPreq == []
        )  # no user has completed both AVT and ACT for program 3

        EventParticipant.create(user="khatts", event=14)
        attendedPreq = trainedParticipants(3, currentTerm)
        assert attendedPreq == [
            khatts
        ]  # "khatts" has completed both AVT and ACT for program 3
    ...
    with mainDB.atomic() as transaction:
        ...
        with pytest.raises(DoesNotExist):
            EventParticipant.get(
                EventParticipant.user == signedInUser, EventParticipant.event == 2
            )
            ...
            participant = EventParticipant.select().where(
                EventParticipant.event == 2, EventParticipant.user == signedInUser
            )
            assert "agliullovak" in participant

            userRsvp = EventRsvp.select().where(
                EventRsvp.event == 2, EventRsvp.user == signedInUser
            )
            ...
            EventRsvp.delete(
                EventRsvp.user == signedInUser, EventRsvp.event == 2
            ).execute()
        mainDB.rollback()
...
def test_getUserParticipatedEvents():
    with mainDB.atomic() as transaction:
        ...
        ProgramEvent.create(program=Program.get_by_id(8), event=testingEvent)
        ...
        listOfProgramTrainings = [
            programTraining for programTraining in allProgramTrainings
        ]
        ...
        programTrainings = getUserParticipatedEvents(
            Program.get_by_id(2), User.get_by_id("ramsayb2"), currentTerm
        )
        ...
        allProgramTrainings = (
            Event.select()
            .join(ProgramEvent)
            .where(Event.isTraining == 1, ProgramEvent.program == Program.get_by_id(8))
        )
        listOfProgramTrainings = [
            programTraining for programTraining in allProgramTrainings
        ]
        programTrainings = getUserParticipatedEvents(
            Program.get_by_id(8), User.get_by_id("ramsayb2"), currentTerm
        )
        for training in programTrainings.keys():
            assert programTrainings[training][0] == None
            assert programTrainings[training][1] == datetime.strptime(
                testingEvent.startDate, "%Y-%m-%d"
            ).strftime("%m/%d/%Y")
        transaction.rollback()
