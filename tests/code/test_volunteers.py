...
from flask import g
from app import app
from app.logic.volunteers import (
    getEventLengthInHours,
    updateEventParticipants,
    addUserBackgroundCheck,
)
...
def test_updateEventParticipants():
    ...
    eventParticipant = EventParticipant.get(
        EventParticipant.user == "agliullovak", EventParticipant.event == 3
    )
    ...
    with pytest.raises(DoesNotExist):
        EventParticipant.get(
            EventParticipant.user == "partont", EventParticipant.event == 3
        )

    participantData = {
        "inputHours_partont": 100,
        "checkbox_partont": "on",
        "event": 3,
        "username1": "partont",
    }
...
def test_backgroundCheck():
    with mainDB.atomic() as transaction:
        with app.app_context():
            ...
            updatebackground = addUserBackgroundCheck(
                "khatts", "CAN", "Submitted", parser.parse("2020-07-20")
            )
            updatedModel = list(
                BackgroundCheck.select().where(
                    BackgroundCheck.user == "khatts", BackgroundCheck.type == "CAN"
                )
            )
            ...
            updatebackground = addUserBackgroundCheck(
                "mupotsal", "SHS", "Failed", parser.parse("2021-07-20")
            )
            updatedModel = list(
                BackgroundCheck.select().where(
                    BackgroundCheck.user == "mupotsal", BackgroundCheck.type == "SHS"
                )
            )
            ...
            updatebackground = addUserBackgroundCheck(
                "khatts", "FBI", "Passed", parser.parse("1999-07-20")
            )
            ...
            updatebackground = addUserBackgroundCheck(
                "khatts", "SHS", "Submitted", parser.parse("2019-07-20")
            )
            ...
            updatebackground = addUserBackgroundCheck(
                "neillz", "FBI", "Passed", parser.parse("2009-07-20")
            )
