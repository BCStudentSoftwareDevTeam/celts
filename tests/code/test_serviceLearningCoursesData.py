...
from app.logic.serviceLearningCoursesData import (
    withdrawProposal,
    renewProposal,
    getServiceLearningCoursesData,
)
...
def test_withdrawProposal():
    ...
    qnote = QuestionNote.create(id=99, question=99, note=99)
    instructor = CourseInstructor.create(id=99, course=99, user="ramsayb2")
    participant = CourseParticipant.create(course=99, user="neillz", hoursEarned=2.0)
...
def test_renewProposal():
    with mainDB.atomic() as transaction:
        ...
        duplicateQuestion = list(
            CourseQuestion.select()
            .join(Course)
            .where(
                CourseQuestion.questionContent == question.questionContent,
        ...
        duplicateInstructor = list(
            CourseInstructor.select()
            .join(Course)
            .where(
                CourseInstructor.user == instructor.user,
                Course.courseName == course.courseName,
            )
        )
