...
def test_banUser():
    with mainDB.atomic() as transaction:
        ...
        prg2BannedUsers = list(
            User.select().join(ProgramBan).where(ProgramBan.program == program_id)
        )
...
def test_Add_Remove_ProgramManager():
    with mainDB.atomic() as transaction:
        ...
        PMsWithNewPM = list(
            User.select().join(ProgramManager).where(ProgramManager.program_id == 1)
        )
...
def test_getStudentManagerForEvent():
    with mainDB.atomic() as transaction:
        ...
        Event.insert_many(
            testEvent
        ).on_conflict_replace().execute()  # Inserts new row into Event table
        ...
        testProgramManagerData = [{"user": "testUser2", "program": 13}]

        # Insert new row into StudentManager table
        ProgramManager.insert_many(
    ...
    with mainDB.atomic() as transaction:
        with app.app_context():
            ...
            addUserBackgroundCheck(
                "usrtst", "CAN", "Submitted", parser.parse("2020-9-20")
            )
            addUserBackgroundCheck(
                "usrtst", "SHS", "Submitted", parser.parse("2020-10-20")
            )
            ...
            assert {
                "CAN": ["Submitted: 09/20/2020"],
                "FBI": [],
                "SHS": ["Submitted: 10/20/2020"],
            } == getUserBGCheckHistory(testusr)
            ...
            addUserBackgroundCheck(
                "usrtst", "SHS", "Passed", parser.parse("2020-12-20")
            )
            assert {
                "CAN": ["Submitted: 09/20/2020"],
                "FBI": [],
                "SHS": ["Passed: 12/20/2020", "Submitted: 10/20/2020"],
            } == getUserBGCheckHistory(testusr)
