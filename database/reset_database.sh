...
mysql -h  mysqldb -u root -proot --execute="DROP DATABASE \`celts\`; DROP USER 'celts_user';"

echo "Recreating databases and users"
mysql -h  mysqldb -u root -proot --execute="CREATE DATABASE IF NOT EXISTS \`celts\`; CREATE USER IF NOT EXISTS 'celts_user'@'%' IDENTIFIED BY 'password'; GRANT ALL PRIVILEGES ON *.* TO 'celts_user'@'%';"
...
if [ $BACKUP -eq 1 ]; then
    echo " from backup"
    mysql -h  mysqldb -u root -proot celts < prod-backup.sql
else
    echo " empty"
    ./migrate_db.sh
fi
