...
class User(baseModel):
    ...
    def removeProgramManager(self, program):
        ...
        ProgramManager.delete().where(
            ProgramManager.user == self, ProgramManager.program == program
        ).execute()
    ...
    def isProgramManagerFor(self, program):
        ...
        from app.models.programManager import (
            ProgramManager,
        )  # Must defer import until now to avoid circular reference

        if self._pmCache is None:
            self._pmCache = (
                ProgramManager.select()
                .where(ProgramManager.user == self, ProgramManager.program == program)
                .exists()
            )
