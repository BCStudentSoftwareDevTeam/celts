...
@serviceLearning_bp.route(
    "/serviceLearning/courseManagement/<username>", methods=["GET"]
)
def serviceCourseManagement(username=None):
    ...
    if g.current_user.isCeltsAdmin or g.current_user.isFaculty:
        ...
        return render_template(
            "serviceLearning/slcManagement.html",
            user=user,
...
def slcEditProposal(courseID):
    ...
    courseInstructor = CourseInstructor.select().where(
        CourseInstructor.course == courseID
    )
    ...
    return render_template(
        "serviceLearning/slcNewProposal.html",
        course=course,
...
def updateInstructorPhone():
    ...
    (
        User.update(phoneNumber=instructorData[1]).where(
            User.username == instructorData[0]
        )
    ).execute()
    return "success"
...
@serviceLearning_bp.route(
    "/serviceLearning/renew/<courseID>/<termID>/", methods=["POST"]
)
