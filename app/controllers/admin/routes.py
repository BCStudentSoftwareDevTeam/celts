...
def switchUser():
    if app.env == "production":
        print(
            f"An attempt was made to switch to another user by {g.current_user.username}!"
        )
        abort(403)
...
@admin_bp.route(
    "/eventTemplates/<templateid>/<programid>/create", methods=["GET", "POST"]
)
def createEvent(templateid, programid=None):
    ...
    try:
        ...
        flash(
            "There was an error with your selection. Please try again or contact Systems Support.",
            "danger",
        )
    ...
    if request.method == "POST":
        try:
            saveSuccess, validationErrorMessage = attemptSaveEvent(
                eventData, attachmentFiles
            )
            createLog(
                f"Created event: {eventData['name']}, which had a start date of {datetime.strftime(eventData['startDate'], '%m/%d/%Y')}"
            )
...
def eventDisplay(eventId):
    if request.method == "POST" and not (
        g.current_user.isCeltsAdmin or g.current_user.isProgramManagerForEvent(eventId)
    ):
        abort(403)
    ...
    if "edit" in rule.rule:
        ...
        eventSeriesList = list(
            Event.select().where(Event.recurringId == event.recurringId)
        )
...
def addRecurringEvents():
    recurringEvents = calculateRecurringEventFrequency(
        preprocessEventData(request.form.copy())
    )
...
def addParticipants():
    ...
    return render_template("addParticipants.html", title="Add Participants")
...
def adminLogs():
    if g.current_user.isCeltsAdmin:
        allLogs = (
            AdminLogs.select(AdminLogs, User)
            .join(User)
            .order_by(AdminLogs.createdOn.desc())
        )
        return render_template("/admin/adminLogs.html", allLogs=allLogs)
    else:
        abort(403)
