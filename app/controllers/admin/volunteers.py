...
def trackVolunteersPage(eventID):
    ...
    if not (
        g.current_user.isCeltsAdmin
        or (g.current_user.isCeltsStudentStaff and isProgramManager)
    ):
        abort(403)

    eventRsvpData = list(EventRsvp.select().where(EventRsvp.event == event))
    eventParticipantData = list(
        EventParticipant.select().where(EventParticipant.event == event)
    )
...
def removeVolunteerFromEvent(user, eventID):
    (
        EventParticipant.delete().where(
            EventParticipant.user == user, EventParticipant.event == eventID
        )
    ).execute()
    (
        EventRsvp.delete().where(EventRsvp.user == user, EventRsvp.event == eventID)
    ).execute()
...
def updateProgramManager():
    if g.current_user.isCeltsAdmin:
        ...
        createLog(
            f'{username.firstName} has been {data["action"]}ed as a Program Manager for {event.name}'
        )
        return ""
    else:
        abort(403)
...
def updatePhone():
    ...
    User.update(phoneNumber=newinfo["phoneNumber"]).where(
