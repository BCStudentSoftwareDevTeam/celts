...
from app.logic.users import (
    ...
    getUserBGCheckHistory,
)
from app.logic.participants import (
    unattendedRequiredEvents,
    trainedParticipants,
...
def events(selectedTerm):
    ...
    participantRSVP = (
        EventRsvp.select(EventRsvp, Event)
        .join(Event)
        .where(EventRsvp.user == g.current_user)
    )
...
def viewUsersProfile(username):
    ...
    try:
        ...
        if g.current_user.isAdmin:
            ...
            abort(
                403
            )  # Error 403 if non admin/student-staff user trys to access via url
    ...
    if (g.current_user == volunteer) or g.current_user.isAdmin:
        ...
        interests = (
            Interest.select(Interest, Program)
            .join(Program)
            .where(Interest.user == volunteer)
        )
        ...
        rsvpedEventsList = (
            EventRsvp.select(EventRsvp, Event)
            .join(Event)
            .where(EventRsvp.user == volunteer)
        )
        rsvpedEvents = [event.event.id for event in rsvpedEventsList]

        programManagerPrograms = (
            ProgramManager.select(ProgramManager, Program)
            .join(Program)
            .where(ProgramManager.user == volunteer)
        )
        ...
        for program in programs:
            notes = list(
                ProgramBan.select(ProgramBan, Note)
                .join(Note, on=(ProgramBan.banNote == Note.id))
                .where(
                    ProgramBan.user == volunteer,
            ...
            userParticipatedEvents = getUserParticipatedEvents(
                program, g.current_user, g.current_term
            )
            allTrainingsComplete = not len(
                [event for event in userParticipatedEvents.values() if event != True]
            )
            noteForDict = notes[-1].banNote.noteContent if notes else ""
            eligibilityTable.append(
                {
                    "program": program,
...
def ban(program_id, username):
    ...
    banEndDate = postData[
        "endDate"
    ]  # Contains the date the ban will no longer be effective
    try:
        ...
        createLog(
            f"Banned {username} from {programInfo.programName} until {banEndDate}."
        )
...
def addInterest(program_id, username):
    ...
    try:
        ...
        if success:
            flash(
                "Successfully added "
                + Program.get_by_id(program_id).programName
                + " as an interest",
                "success",
            )
            return ""
...
def volunteerRegister():
    ...
    if isEligible:
        ...
        if personAdded and listOfRequirements:
            ...
            flash(
                f"{user.firstName} {user.lastName} successfully registered. However, the following training may be required: {reqListToString}.",
                "success",
            )
        elif personAdded:
        ...
        flash(
            f"Cannot RSVP. Contact CELTS administrators: {app.config['celts_admin_contact']}.",
            "danger",
        )
...
def RemoveRSVP():
    ...
    currentRsvpParticipant = EventRsvp.get(
        EventRsvp.user == g.current_user, EventRsvp.event == event
    )
...
def reviewProposal():
    ...
    return render_template(
        "/main/reviewproposal.html", course=course, instructors_data=instructors_data
    )
...
def getAllCourseInstructors(term=None):
    ...
    if g.current_user.isCeltsAdmin:
        ...
        return render_template(
            "/main/manageServiceLearningFaculty.html",
