...
def email():
    ...
    if "@" in raw_form_data["emailSender"]:
        ...
        mail = EmailHandler(
            raw_form_data, url_domain, g.current_user, attachment_file=attachments
        )
        ...
        return redirect(
            url_for("main.events", selectedTerm=raw_form_data["selectedTerm"])
        )
...
def loadKiosk(eventid):
    ...
    return render_template("/events/eventKiosk.html", event=event, eventid=eventid)
...
def signinEvent():
    ...
    programid = ProgramEvent.select(ProgramEvent.program).where(
        ProgramEvent.event == eventid
    )

    if (
        bnumber[0] == ";" and bnumber[-1] == "?"
    ...
    try:
        ...
        if userStatus == "banned":
            ...
            flasherMessage = (
                f"{kioskUser.firstName} {kioskUser.lastName} Already Signed In!"
            )
            return flasherMessage

        else:
            flasherMessage = (
                f"{kioskUser.firstName} {kioskUser.lastName} Successfully Signed In!"
            )
            return flasherMessage
