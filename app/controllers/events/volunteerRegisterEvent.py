...
def volunteerRegister(userid,  eventid):
    ...
    program = Event.select(Event.program).where(Event.id == eventid)
    ...
    if isEligibleForProgram(program, user):
        ...
        if not EventParticipant.select().where(EventParticipant.user == user,
                                           EventParticipant.event == event).exists():
