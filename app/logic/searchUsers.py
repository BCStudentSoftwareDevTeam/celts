...
def searchInstructor(query):
    ...
    if len(splitSearch) == 1: #search for first or last name
        results = User.select().where((User.isSLinstructor | User.isFaculty) & (User.firstName ** firstName | User.lastName ** firstName))
        ...
        for searchTerm in splitSearch: #searching for specified first and last name
            if len(searchTerm) > 1:
                searchTerm += "%"
                results = User.select().where((User.isFaculty | User.isSLinstructor) & User.firstName ** firstName & User.lastName ** lastName)
                for instructor in results:
                    if instructor not in resultsDict:
                        resultsDict[instructor.username]=model_to_dict(instructor)
