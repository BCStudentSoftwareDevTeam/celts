...
class FileHandler:
        ...
        try:
            for file in self.files:
                isFileInEvent = (
                    EventFile.select()
                    .where(
                        EventFile.event == eventId, EventFile.fileName == file.filename
                    )
                    .exists()
                )
                if not isFileInEvent:
                    ...
                    file.save(
                        self.getFileFullPath(eventId=eventId, newfile=file)
                    )  # saves attachment in directory
        except AttributeError:  # will pass if there is no attachment to save
            return False
            pass
    ...
    def retrievePath(self, files, eventId=None):
        pathDict = {}
        for file in files:
            pathDict[file.fileName] = (
                (self.path + "/" + str(eventId) + "/" + file.fileName)[3:],
                file,
            )
        return pathDict
