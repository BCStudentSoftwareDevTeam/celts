...
def getEvents(program_id=None):
    ...
    if program_id:
        ...
        return (
            Event.select(Event)
            .join(ProgramEvent)
            .where(ProgramEvent.program == program_id)
            .distinct()
        )
    else:
        return Event.select()
...
def deleteEvent(eventId):
    ...
    if event:
        ...
        createLog(
            f"Deleted event: {event.name}, which had a start date of {datetime.datetime.strftime(event.startDate, '%m/%d/%Y')}"
        )
...
def saveEventToDb(newEventData):
    ...
    if isNewEvent and newEventData["isRecurring"]:
        ...
        eventsToCreate.append(
            {
                "name": f"{newEventData['name']}",
        ...
        with mainDB.atomic():
            ...
            if isNewEvent:
                ...
                if "program" in newEventData:
                    ProgramEvent.create(
                        program=newEventData["program"], event=eventRecord
                    )
            else:
...
def getStudentLedEvents(term):
    ...
    studentLedEvents = list(
        Event.select(Event, Program, ProgramEvent)
        .join(ProgramEvent, attr="programEvent")
        .join(Program)
        .where(Program.isStudentLed, Event.term == term)
        .execute()
    )
    programs = {}
...
def getBonnerEvents(term):
    ...
    bonnerScholarsEvents = list(
        Event.select(Event, ProgramEvent, Program.id.alias("program_id"))
        .join(ProgramEvent)
        .join(Program)
        .where(Program.isBonnerScholars, Event.term == term)
        .execute()
    )
...
def getUpcomingEventsForUser(user, asOf=datetime.datetime.now()):
    ...
    events = list(
        ...
        .where(
            Event.startDate >= asOf, (Interest.user == user) | (EventRsvp.user == user)
        )
        .distinct()  # necessary because of multiple programs
        .order_by(Event.startDate, Event.name)
        .execute()  # keeps the order of events the same when the dates are the same
    )
...
def getParticipatedEventsForUser(user):
    ...
    participatedEvents = (
        Event.select(Event, Program.programName)
        .join(ProgramEvent, JOIN.LEFT_OUTER)
        .join(Program, JOIN.LEFT_OUTER)
        .switch()
        .join(EventParticipant)
        .where(EventParticipant.user == user, Event.isAllVolunteerTraining == False)
        .order_by(Event.startDate, Event.name)
    )
    ...
    allVolunteer = (
        Event.select(Event, "")
        .join(EventParticipant)
        .where(Event.isAllVolunteerTraining == True, EventParticipant.user == user)
    )
    union = participatedEvents.union_all(allVolunteer)
    unionParticipationWithVolunteer = list(
        union.select_from(
            union.c.id, union.c.programName, union.c.startDate, union.c.name
        )
        .order_by(union.c.startDate, union.c.name)
        .execute()
    )
...
def calculateRecurringEventFrequency(event):
    ...
    """
    if not isinstance(event["endDate"], datetime.date) or not isinstance(
    ...
    return [
        {
            "name": f"{event['name']} Week {counter+1}",
