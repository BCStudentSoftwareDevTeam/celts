...
def getStartofCurrentAcademicYear(currentTerm):
    if ("Summer" in currentTerm.description) or ("Spring" in currentTerm.description):
        fallTerm = (
            Term.select()
            .where(
                Term.year == currentTerm.year - 1,
                Term.description == f"Fall {currentTerm.year-1}",
            )
            .get()
        )
        return fallTerm
...
def format24HourTime(unformattedTime):
    ...
    if type(unformattedTime) == str:
        try:
            formattedTime = datetime.strptime(unformattedTime, "%I:%M %p").strftime(
                "%H:%M"
            )  # Converts string to datetime then back to string and formats correctly
