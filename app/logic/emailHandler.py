...
class EmailHandler:
    ...
    def process_data(self):
        ...
        if "programID" in self.raw_form_data:
            self.program_ids = self.fetch_event_programs(
                self.raw_form_data["programID"]
            )
    ...
    def fetch_event_programs(self, program_id):
        ...
        if program_id == "Unknown" or program_id is None:
            programEvents = ProgramEvent.select(ProgramEvent.program).where(
                ProgramEvent.event == self.event.id
            )
    ...
    def retrieve_recipients(self, recipients_category):
        ...
        if recipients_category == "RSVP'd":
            recipients = (
                User.select().join(EventRsvp).where(EventRsvp.event == self.event.id)
            )
        ...
        if recipients_category == "Eligible Students":
            ...
            sameYearTerms = (
                Term.select()
                .join(Term2, on=(Term.academicYear == Term2.academicYear))
                .where(Term2.isCurrentTerm == True)
            )

            bannedUsers = ProgramBan.select(ProgramBan.user_id).where(
                (ProgramBan.endDate > datetime.now()) | (ProgramBan.endDate is None),
                ProgramBan.program_id.in_([p.id for p in self.program_ids]),
            )
            allVolunteer = Event.select().where(
                Event.isAllVolunteerTraining == True, Event.term.in_(sameYearTerms)
            )
            recipients = (
                User.select()
                .join(EventParticipant)
                .where(
                    User.username.not_in(bannedUsers),
                    EventParticipant.event.in_(allVolunteer),
                )
            )
    ...
    def retrieve_and_modify_email_template(self):
        ...
        email_template = EmailTemplate.get(
            EmailTemplate.purpose == self.template_identifier
        )  # --Q: should we keep purpose as the identifier?
                ...
                for recipient in self.recipients:
                    ...
                    conn.send(
                        Message(
                            ...
                            sender=(
                                defaultEmailInfo["senderName"],
                                defaultEmailInfo["replyTo"],
                            ),
                        )
                    )
    ...
    def update_email_template(self):
        try:
            ...
            (
                ...
                ).where(EmailTemplate.purpose == self.template_identifier)
            ).execute()
            return True
        except Exception as e:
    ...
    def retrieve_last_email(event_id):
        try:
            last_email = (
                EmailLog.select()
                .where(EmailLog.event == event_id)
                .order_by(EmailLog.dateSent.desc())
                .get()
            )
            return last_email
        except DoesNotExist:
